...
def plot_tracking(image, tlwhs, obj_ids, scores=None, frame_id=0, fps=0., ids2=None, selected_id=None, click_pos=[], is_selected=False):
    ...
    for i, tlwh in enumerate(tlwhs):
        ...
        # check if there is a mouse event happend
        if len(click_pos) > 0:
            # check which object is selected           
            if (intbox[0] < click_pos[0] < intbox[2]) and (intbox[1] < click_pos[1] < intbox[3]):
                if not is_selected and len(click_pos) > 0:
                    # set selected mode
                    selected_id = obj_id
                    is_selected = True
                    click_pos = []

        # set object id to bbox
    ...
    return im, click_pos, selected_id, is_selected
